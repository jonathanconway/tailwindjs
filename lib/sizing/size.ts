/**
 * size-0
 *
 * CSS properties:
 * - width: 0px;, * - height: 0px;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_0 = "size-0";

/**
 * size-px
 *
 * CSS properties:
 * - width: 1px;, * - height: 1px;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_px = "size-px";

/**
 * size-0.5
 *
 * CSS properties:
 * - width: 0.125rem; \/* 2px *\/, * - height: 0.125rem; \/* 2px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_0_5 = "size-0.5";

/**
 * size-1
 *
 * CSS properties:
 * - width: 0.25rem; \/* 4px *\/, * - height: 0.25rem; \/* 4px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_1 = "size-1";

/**
 * size-1.5
 *
 * CSS properties:
 * - width: 0.375rem; \/* 6px *\/, * - height: 0.375rem; \/* 6px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_1_5 = "size-1.5";

/**
 * size-2
 *
 * CSS properties:
 * - width: 0.5rem; \/* 8px *\/, * - height: 0.5rem; \/* 8px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_2 = "size-2";

/**
 * size-2.5
 *
 * CSS properties:
 * - width: 0.625rem; \/* 10px *\/, * - height: 0.625rem; \/* 10px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_2_5 = "size-2.5";

/**
 * size-3
 *
 * CSS properties:
 * - width: 0.75rem; \/* 12px *\/, * - height: 0.75rem; \/* 12px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_3 = "size-3";

/**
 * size-3.5
 *
 * CSS properties:
 * - width: 0.875rem; \/* 14px *\/, * - height: 0.875rem; \/* 14px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_3_5 = "size-3.5";

/**
 * size-4
 *
 * CSS properties:
 * - width: 1rem; \/* 16px *\/, * - height: 1rem; \/* 16px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_4 = "size-4";

/**
 * size-5
 *
 * CSS properties:
 * - width: 1.25rem; \/* 20px *\/, * - height: 1.25rem; \/* 20px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_5 = "size-5";

/**
 * size-6
 *
 * CSS properties:
 * - width: 1.5rem; \/* 24px *\/, * - height: 1.5rem; \/* 24px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_6 = "size-6";

/**
 * size-7
 *
 * CSS properties:
 * - width: 1.75rem; \/* 28px *\/, * - height: 1.75rem; \/* 28px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_7 = "size-7";

/**
 * size-8
 *
 * CSS properties:
 * - width: 2rem; \/* 32px *\/, * - height: 2rem; \/* 32px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_8 = "size-8";

/**
 * size-9
 *
 * CSS properties:
 * - width: 2.25rem; \/* 36px *\/, * - height: 2.25rem; \/* 36px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_9 = "size-9";

/**
 * size-10
 *
 * CSS properties:
 * - width: 2.5rem; \/* 40px *\/, * - height: 2.5rem; \/* 40px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_10 = "size-10";

/**
 * size-11
 *
 * CSS properties:
 * - width: 2.75rem; \/* 44px *\/, * - height: 2.75rem; \/* 44px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_11 = "size-11";

/**
 * size-12
 *
 * CSS properties:
 * - width: 3rem; \/* 48px *\/, * - height: 3rem; \/* 48px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_12 = "size-12";

/**
 * size-14
 *
 * CSS properties:
 * - width: 3.5rem; \/* 56px *\/, * - height: 3.5rem; \/* 56px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_14 = "size-14";

/**
 * size-16
 *
 * CSS properties:
 * - width: 4rem; \/* 64px *\/, * - height: 4rem; \/* 64px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_16 = "size-16";

/**
 * size-20
 *
 * CSS properties:
 * - width: 5rem; \/* 80px *\/, * - height: 5rem; \/* 80px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_20 = "size-20";

/**
 * size-24
 *
 * CSS properties:
 * - width: 6rem; \/* 96px *\/, * - height: 6rem; \/* 96px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_24 = "size-24";

/**
 * size-28
 *
 * CSS properties:
 * - width: 7rem; \/* 112px *\/, * - height: 7rem; \/* 112px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_28 = "size-28";

/**
 * size-32
 *
 * CSS properties:
 * - width: 8rem; \/* 128px *\/, * - height: 8rem; \/* 128px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_32 = "size-32";

/**
 * size-36
 *
 * CSS properties:
 * - width: 9rem; \/* 144px *\/, * - height: 9rem; \/* 144px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_36 = "size-36";

/**
 * size-40
 *
 * CSS properties:
 * - width: 10rem; \/* 160px *\/, * - height: 10rem; \/* 160px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_40 = "size-40";

/**
 * size-44
 *
 * CSS properties:
 * - width: 11rem; \/* 176px *\/, * - height: 11rem; \/* 176px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_44 = "size-44";

/**
 * size-48
 *
 * CSS properties:
 * - width: 12rem; \/* 192px *\/, * - height: 12rem; \/* 192px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_48 = "size-48";

/**
 * size-52
 *
 * CSS properties:
 * - width: 13rem; \/* 208px *\/, * - height: 13rem; \/* 208px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_52 = "size-52";

/**
 * size-56
 *
 * CSS properties:
 * - width: 14rem; \/* 224px *\/, * - height: 14rem; \/* 224px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_56 = "size-56";

/**
 * size-60
 *
 * CSS properties:
 * - width: 15rem; \/* 240px *\/, * - height: 15rem; \/* 240px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_60 = "size-60";

/**
 * size-64
 *
 * CSS properties:
 * - width: 16rem; \/* 256px *\/, * - height: 16rem; \/* 256px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_64 = "size-64";

/**
 * size-72
 *
 * CSS properties:
 * - width: 18rem; \/* 288px *\/, * - height: 18rem; \/* 288px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_72 = "size-72";

/**
 * size-80
 *
 * CSS properties:
 * - width: 20rem; \/* 320px *\/, * - height: 20rem; \/* 320px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_80 = "size-80";

/**
 * size-96
 *
 * CSS properties:
 * - width: 24rem; \/* 384px *\/, * - height: 24rem; \/* 384px *\/
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_96 = "size-96";

/**
 * size-auto
 *
 * CSS properties:
 * - width: auto;, * - height: auto;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_auto = "size-auto";

/**
 * size-1/2
 *
 * CSS properties:
 * - width: 50%;, * - height: 50%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_1_on_2 = "size-1/2";

/**
 * size-1/3
 *
 * CSS properties:
 * - width: 33.333333%;, * - height: 33.333333%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_1_on_3 = "size-1/3";

/**
 * size-2/3
 *
 * CSS properties:
 * - width: 66.666667%;, * - height: 66.666667%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_2_on_3 = "size-2/3";

/**
 * size-1/4
 *
 * CSS properties:
 * - width: 25%;, * - height: 25%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_1_on_4 = "size-1/4";

/**
 * size-2/4
 *
 * CSS properties:
 * - width: 50%;, * - height: 50%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_2_on_4 = "size-2/4";

/**
 * size-3/4
 *
 * CSS properties:
 * - width: 75%;, * - height: 75%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_3_on_4 = "size-3/4";

/**
 * size-1/5
 *
 * CSS properties:
 * - width: 20%;, * - height: 20%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_1_on_5 = "size-1/5";

/**
 * size-2/5
 *
 * CSS properties:
 * - width: 40%;, * - height: 40%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_2_on_5 = "size-2/5";

/**
 * size-3/5
 *
 * CSS properties:
 * - width: 60%;, * - height: 60%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_3_on_5 = "size-3/5";

/**
 * size-4/5
 *
 * CSS properties:
 * - width: 80%;, * - height: 80%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_4_on_5 = "size-4/5";

/**
 * size-1/6
 *
 * CSS properties:
 * - width: 16.666667%;, * - height: 16.666667%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_1_on_6 = "size-1/6";

/**
 * size-2/6
 *
 * CSS properties:
 * - width: 33.333333%;, * - height: 33.333333%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_2_on_6 = "size-2/6";

/**
 * size-3/6
 *
 * CSS properties:
 * - width: 50%;, * - height: 50%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_3_on_6 = "size-3/6";

/**
 * size-4/6
 *
 * CSS properties:
 * - width: 66.666667%;, * - height: 66.666667%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_4_on_6 = "size-4/6";

/**
 * size-5/6
 *
 * CSS properties:
 * - width: 83.333333%;, * - height: 83.333333%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_5_on_6 = "size-5/6";

/**
 * size-1/12
 *
 * CSS properties:
 * - width: 8.333333%;, * - height: 8.333333%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_1_on_12 = "size-1/12";

/**
 * size-2/12
 *
 * CSS properties:
 * - width: 16.666667%;, * - height: 16.666667%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_2_on_12 = "size-2/12";

/**
 * size-3/12
 *
 * CSS properties:
 * - width: 25%;, * - height: 25%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_3_on_12 = "size-3/12";

/**
 * size-4/12
 *
 * CSS properties:
 * - width: 33.333333%;, * - height: 33.333333%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_4_on_12 = "size-4/12";

/**
 * size-5/12
 *
 * CSS properties:
 * - width: 41.666667%;, * - height: 41.666667%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_5_on_12 = "size-5/12";

/**
 * size-6/12
 *
 * CSS properties:
 * - width: 50%;, * - height: 50%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_6_on_12 = "size-6/12";

/**
 * size-7/12
 *
 * CSS properties:
 * - width: 58.333333%;, * - height: 58.333333%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_7_on_12 = "size-7/12";

/**
 * size-8/12
 *
 * CSS properties:
 * - width: 66.666667%;, * - height: 66.666667%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_8_on_12 = "size-8/12";

/**
 * size-9/12
 *
 * CSS properties:
 * - width: 75%;, * - height: 75%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_9_on_12 = "size-9/12";

/**
 * size-10/12
 *
 * CSS properties:
 * - width: 83.333333%;, * - height: 83.333333%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_10_on_12 = "size-10/12";

/**
 * size-11/12
 *
 * CSS properties:
 * - width: 91.666667%;, * - height: 91.666667%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_11_on_12 = "size-11/12";

/**
 * size-full
 *
 * CSS properties:
 * - width: 100%;, * - height: 100%;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_full = "size-full";

/**
 * size-min
 *
 * CSS properties:
 * - width: min-content;, * - height: min-content;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_min = "size-min";

/**
 * size-max
 *
 * CSS properties:
 * - width: max-content;, * - height: max-content;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_max = "size-max";

/**
 * size-fit
 *
 * CSS properties:
 * - width: fit-content;, * - height: fit-content;
 *
 * @see https://tailwindcss.com/docs/size
 *
 * @type utility
 */
export const size_fit = "size-fit";

/**
 * size-[:value]
 *
 * If you need to use a one-off size value that doesn’t make sense to include in your theme, use square brackets to generate a property on the fly using any arbitrary value.
 *
 * @see https://tailwindcss.com/docs/size#arbitrary-values
 */
export const size_arbitrary = (value: string) => `size-[${value}]`;

/**
 * Sizing
 *
 * Utilities for setting the width and height of an element at the same time.
 *
 * @see https://tailwindcss.com/docs/size
 */
export const size = {
  size_0,
  size_px,
  size_0_5,
  size_1,
  size_1_5,
  size_2,
  size_2_5,
  size_3,
  size_3_5,
  size_4,
  size_5,
  size_6,
  size_7,
  size_8,
  size_9,
  size_10,
  size_11,
  size_12,
  size_14,
  size_16,
  size_20,
  size_24,
  size_28,
  size_32,
  size_36,
  size_40,
  size_44,
  size_48,
  size_52,
  size_56,
  size_60,
  size_64,
  size_72,
  size_80,
  size_96,
  size_auto,
  size_1_on_2,
  size_1_on_3,
  size_2_on_3,
  size_1_on_4,
  size_2_on_4,
  size_3_on_4,
  size_1_on_5,
  size_2_on_5,
  size_3_on_5,
  size_4_on_5,
  size_1_on_6,
  size_2_on_6,
  size_3_on_6,
  size_4_on_6,
  size_5_on_6,
  size_1_on_12,
  size_2_on_12,
  size_3_on_12,
  size_4_on_12,
  size_5_on_12,
  size_6_on_12,
  size_7_on_12,
  size_8_on_12,
  size_9_on_12,
  size_10_on_12,
  size_11_on_12,
  size_full,
  size_min,
  size_max,
  size_fit,
  size_arbitrary
};